...
async def search_and_review_setup(client: AsyncClient) -> dict:
    ...
    async with TestingSessionLocal() as session:
        ...
        p1 = Product(
            ...
            slug=f"pro-wireless-headphones-{uuid.uuid4().hex[:4]}",
            sku=f"SKU-HEADPHONE-{uuid.uuid4().hex[:4]}",
        ...
        p2 = Product(
            ...
            slug=f"ultra-gaming-keyboard-{uuid.uuid4().hex[:4]}",
            sku=f"SKU-KEYBOARD-{uuid.uuid4().hex[:4]}",
        ...
        p3 = Product(
            ...
            slug=f"ergonomic-gaming-mouse-{uuid.uuid4().hex[:4]}",
            sku=f"SKU-MOUSE-{uuid.uuid4().hex[:4]}",
